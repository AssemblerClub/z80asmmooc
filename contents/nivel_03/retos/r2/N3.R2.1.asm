...
run #4000      ;; Ejecuta en 4000 cuando ensambla
   ...
   ;;
   ;; VALOR PARA DECIDIR SI SE MUEVE O NO LA VAGONETA
   ;;
   ld  a, 0       ;; Valor para decidir si hay que mover la vagoneta o no (1=mover)
   ...
   ;;
   ld  b, 1       ;; Caso base: sólo 1 repetición del bucle de dibujado (no mover)
   ld  e, 1       ;; Ciclos halt de espera entre fotogramas

   dec a          ;; Decrementamos A para comprobar 
   jr nz, nomover ;; Si A!=0, no moveremos la vagoneta (1 repetición del bucle)

   ;; A = 0. Tenemos que mover la vagoneta. Cargamos las repeticiones del bucle
...
nomover:
   ...
   dibujado:
      ...
      ld    c, e     ;; C = número de halts a esperar
      espera:
         halt        ;; Esperamos 1/300 de segundo
         dec c       ;; Descontamos una unidad de espera
      jr nz, espera  ;; Si aún quedan unidades de espera, repetimos
      inc   e        ;; En la próxima repetición, esperaremos 1 halt más.

      dec b          ;; Una repetición menos de la animación
   jp nz, dibujado   ;; Si aún quedan frames de la animación, repetir
